	...
	public void run() {
		...
		while (keepRunning) {
			try {
				...
				if (msg.type.equals("secretkey")) {
					key = msg.content;
					ui.jTextArea1.append("Secret key recieved!");
				} else if (msg.type.equals("message")) {
					msg.content = AES.decrypt(msg.content);
					...
					if (msg.content.equals("TRUE")) {
						...
						send(new Message("secretkey", msg.recipient, "gettingkey", msg.sender));
					} else {
	...
	public void send(Message msg) {
		try {
			if (msg.type.equals("message")) {
				msg.content = AES.encrypt(msg.content);
			}
			Out.writeObject(msg);
			Out.flush();
